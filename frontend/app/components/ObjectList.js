...
export default function ObjectList() {
  ...
  const getObject = async (key) => {
    await apiRequest("/usage/storage/get", "POST", { object_key: key });
    alert(`GET event recorded for ${key}`);
  };

  const deleteObject = async (key) => {
    await apiRequest("/usage/storage/delete", "POST", { object_key: key });
  ...
  return (
    <div className="flex flex-col">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
            onClick={loadObjects} 
            className="flex items-center text-sm px-4 py-2 bg-gray-100 text-gray-700 hover:bg-gray-200 hover:text-gray-900 border border-gray-200 rounded-lg transition-colors font-medium shadow-none"
        >
      ...
      <div className="overflow-x-auto bg-white rounded-xl border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
                <tr>
                    <th scope="col" className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Object Key</th>
                    <th scope="col" className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Size (MB)</th>
                    ...
                    <tr key={i} className="hover:bg-sky-50 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                                ...
                                <span className="text-sm font-medium text-gray-900">{obj.object_key}</span>
                            </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button 
                                onClick={() => getObject(obj.object_key)}
                            ...
                            <button 
                                onClick={() => deleteObject(obj.object_key)}
        ...
        {objects.length === 0 && (
            <div className="px-6 py-12 text-center text-gray-500 text-sm flex flex-col items-center">
